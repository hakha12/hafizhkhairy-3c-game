...
public class PlayerMovement : MonoBehaviour {
	...
	private void Move(Vector2 axisDirection){
		...
		if ((isPlayerStanding || isPlayerCrouching) && !_isPunching){
			...
			Vector3 rotationDegrees = transform.rotation.eulerAngles;

			rotationDegrees.x += _glideRotationSpeed.x * axisDirection.y * Time.deltaTime;
			rotationDegrees.y += _glideRotationSpeed.y * axisDirection.x;
			rotationDegrees.z += _glideRotationSpeed.z * axisDirection.x;

			rotationDegrees.x = Mathf.Clamp(rotationDegrees.x, _minGlideRotationX, _maxGlideRotationX);

			transform.rotation = Quaternion.Euler(rotationDegrees);
		}
	...
	private void Punch(){
		if (_isPunching || _playerStance != PlayerStance.Stand) return;
